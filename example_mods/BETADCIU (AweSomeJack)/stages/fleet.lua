...
function onCreate()
	...
	addCharacterToList('luacle-dark', 'boyfriend')
	precacheSound('stat')

	setPropertyFromClass('GameOverSubstate', 'characterName', 'luacle-phantasm')
	setPropertyFromClass('GameOverSubstate', 'loopSoundName', 'gameOver-phantasm');
end
...
function fleetON()
	...
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'luacle-dark')
end
...
function fleetOFF()
	...
	doTweenAlpha('fleetON', 'fx', 0, 1, 'linear')
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'luacle-phantasm')
end
