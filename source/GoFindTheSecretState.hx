...
class GoFindTheSecretState extends MusicBeatState
{
	...
	{
		FlxG.sound.playMusic(Paths.music('LOL'));

		sonicBG = new FlxSprite(-100, -100).loadGraphic(Paths.image('sorry/goback'));
		...
		lolText.text = 'Hello, you found a hidden song.'
		+ '\n sorry, but now the hidden song is not play. Only released songs can be play.';
