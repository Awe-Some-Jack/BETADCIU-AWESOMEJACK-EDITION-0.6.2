...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if ((SONG.song == 'ballistic' || SONG.song == 'Bopeebo Rumble') && !isHidden)
		{
					...
					switch (curStep)
					{
						...
						case 610:
							if (ratingPercent >= 0.8) {
								...
								dad.playAnim('happy', true);
								dad.specialAnim = true;
							}
						case 626:
							if (ratingPercent >= 0.8) {
								dadGroup.remove(dad);
								dad = new Character(-200, 100, 'luacle-mad');
								add(dad);
							}
							else {
								dadGroup.remove(dad);
								dad = new Character(-500, -40, 'luacle');
								iconP2.changeIcon('luacle');
								add(dad);
								boyfriendGroup.remove(boyfriend);
								boyfriend = new Boyfriend(670, 300, 'spong-sad');
								add(boyfriend);
							}
						case 948:
							if (isStoryMode && ratingPercent >= 0.8) {
